...
def render_tab_alertas(df_diagnostic: pd.DataFrame, df_modelo: pd.DataFrame) -> None:
    ...
    with m2:
        st.markdown(_alert_metric("Placas a revisar", str(high_variance), "desvío alto", COLOR["red"] if high_variance else COLOR["green"]), unsafe_allow_html=True)
    with m3:
        st.markdown(_alert_metric("Mayor desvío", f"{max_variance:.0f}", "en la placa con más variación", "#a45f00"), unsafe_allow_html=True)
    with m4:
        st.markdown(_alert_metric("Desvío medio", f"{avg_variance:.0f}", "movimiento general de las placas", "#007aff"), unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        'letter-spacing:0.06em;margin-bottom:8px;">Revisión de movimiento — todas las placas</div>',
        unsafe_allow_html=True,
    )

    diag_chart = diag_rank.reset_index().rename(columns={"index": "Placa"})
    diag_chart["Estado"] = diag_chart["varianza_deg2"].apply(lambda v: "Movimiento irregular" if v > 450 else "Correcto")
    ...
    fig_var = px.bar(
        ...
        orientation="h",
        color="Estado",
        color_discrete_map={"Movimiento irregular": COLOR["red"], "Correcto": COLOR["green"]},
        title="Estabilidad de placas",
        text=diag_chart["varianza_deg2"].map(lambda v: f"{v:.0f}"),
    )
    fig_var.update_traces(textposition="outside", marker_line_width=0)
    fig_var.add_vline(x=450, line_dash="dash", line_color="#ff3b30", annotation_text="revisar")
    ...
    diag_display["Estado"] = diag_display["varianza_deg2"].apply(
        lambda v: "Movimiento irregular" if v > 450 else "Correcto"
    )
    diag_display = diag_display.rename(columns={
        "varianza_deg2":   "Desvío",
        "posible_stow_fijo": "Posición fija",
    })
    st.dataframe(diag_display, use_container_width=True)
    st.caption("Se marca para revisar cuando el desvío supera 450.")
    ...
    if "VWC_S1_mean" in df_modelo.columns:
        ...
        st.markdown(
            '<div style="font-size:12px;font-weight:760;color:#64706d;text-transform:uppercase;'
            'letter-spacing:0.06em;margin-bottom:8px;">Humedad del suelo — últimas 48h</div>',
            unsafe_allow_html=True,
        )
        ...
        if not last_48h.empty:
            ...
            fig.add_hline(y=20.0, line_dash="dash", line_color="#dc2626",
                          annotation_text="humedad baja", annotation_font_size=9)
