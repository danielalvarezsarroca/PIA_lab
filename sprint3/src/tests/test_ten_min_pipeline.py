...
def _master_sample() -> pd.DataFrame:
    return pd.DataFrame({
        ...
        "ALBEDO_mean": [18.0, 18.5, 19.0, 62.0, 66.0, 64.0],
...
def test_build_modeling_dataset_10min_preserves_resolution_and_maps_columns():
    ...
    six_am = model[model["Time"].dt.hour == 6]
    assert six_am["Time"].diff().dropna().dt.total_seconds().eq(600).all()
...
def test_regenerate_candidate_rules_10min_uses_finer_time_blocks():
    ...
    assert "10min_microbloque" in set(rules["tipo"])
    assert rules["comentario"].str.contains("10 min").any()


def test_regenerate_candidate_rules_10min_includes_conditional_and_period_rules():
    model = build_modeling_dataset_10min(_master_sample())

    rules = regenerate_candidate_rules_10min(model)

    assert "10min_condicional_albedo_elevacion" in set(rules["tipo"])
    assert "10min_franja_manana" in set(rules["tipo"])
    assert "10min_franja_mediodia" in set(rules["tipo"])
    assert rules["regla"].str.contains("Albedo_S1").any()
